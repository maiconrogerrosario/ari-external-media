...
const { startInternalWebSocket } = require('./websocket_server');
...
async function playAudio(client, channel, sound) {
  try {
    logger.info(`Playing audio "${sound}" on channel ${channel.id}`);
    await channel.play({
      media: `sound:${sound}`
    });
  } catch (e) {
    logger.error(`Playback error on channel ${channel.id}: ${e.message}`);
  }
}
    ...
    ariClient.on('StasisStart', async (evt, channel) => {
      ...
      try {
        ...
        await channel.answer();
	      await playAudio(ariClient, channel, 'pt_BR');
        ...
        const extParams = {
          app: config.ARI_APP,
          external_host: `172.31.0.47:${port}`,
          format: 'slin',
          transport: 'udp',
          encapsulation: 'rtp',
        ...
        logger.info(`ExternalMedia channel ${extChannel.id} created with codec ulaw, RTP to 172.31.0.47:${port}`);
		...
		await startInternalWebSocket(channel.id);
      } catch (e) {
